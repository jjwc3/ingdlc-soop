...
/**
 * 도배 문구를 받아 최대 128자로 늘리고 복사하는 함수
 */
const copyToClipboard = async ( val: string ) => {
  ...
  try {
    await navigator.clipboard.writeText( newVal.trim() );
  } catch ( e ) {
    console.error( e );
  }
};
 ...
 */
const copyToClipboardOne = async ( copyText: string ) => {
...
/**
 * str을 받아서, 50자 미만일 경우 그대로 return, 이상일 경우 50자까지 끊고 "..." 붙여서 return
 */
const maxLen = ( str: string ): string => {
  if ( str.length >= 50 ) {
    // return str.substr(0,50)+"..."; // substr is deprecated
...
/**
 * length 자릿수의 수의 맨 끝에 number를 넣고, 남는 자리에 0 채우는 함수. datetime 함수에서만 사용됨.
 */
...
/**
 * 현재 시간을 YYYYMMDD_hhmmss 형식으로 변환해 return
 */
...
export { log, copyToClipboard, copyToClipboardOne, maxLen, datetime, playAlert };
